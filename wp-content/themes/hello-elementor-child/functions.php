...
/**
 * Override loop template add to cart buttons
 */
// remove_filter( 'woocommerce_loop_add_to_cart_link', 'filter_woocommerce_loop_add_to_cart_link', 10, 2 ); 
...
function filter_woocommerce_loop_add_to_cart_link( $args, $product ) {
    ...
    if ( is_product_taxonomy() ) {
        ...
        $quantity = isset( $args['quantity'] ) ? $args['quantity'] : 1;
        $text = "Buy Now";
        $product_custom_color = get_field('shop_page_color', $product_id);
        $product_url .= "?attribute_pa_color"."=". $product_custom_color;
